...
const SPRING: Transition = { type: 'spring', stiffness: 180, damping: 14, mass: 0.8 }
...
function HairLong({ color }: { color: string }) {
  return (
    <>
      <path d="M 16 10 Q 20 2 34 2 Q 48 2 52 10 L 52 22 L 16 22 Z" fill={color} stroke={STROKE} strokeWidth="2" />
      <path d="M 16 18 L 14 34 Q 13 42 16 44 Q 18 43 18 38 L 18 22" fill={color} stroke={STROKE} strokeWidth="1.5" />
      <path d="M 52 18 L 54 34 Q 55 42 52 44 Q 50 43 50 38 L 50 22" fill={color} stroke={STROKE} strokeWidth="1.5" />
...
function HairBob({ color }: { color: string }) {
  return <path d="M 16 12 Q 34 6 52 12 L 50 38 L 18 38 Z" fill={color} stroke={STROKE} strokeWidth="2" />
}
...
function HairPigtails({ color }: { color: string }) {
  return (
    <>
      <path d="M 16 10 Q 20 2 34 2 Q 48 2 52 10 L 52 20 L 16 20 Z" fill={color} stroke={STROKE} strokeWidth="2" />
      <path d="M 16 16 L 10 22 Q 8 28 10 32 L 12 34" fill="none" stroke={color} strokeWidth="5" strokeLinecap="round" />
      ...
      <path d="M 52 16 L 58 22 Q 60 28 58 32 L 56 34" fill="none" stroke={color} strokeWidth="5" strokeLinecap="round" />
...
function HairWavy({ color }: { color: string }) {
  return (
    <>
      <path d="M 16 12 Q 20 4 34 4 Q 48 4 52 12 L 52 22 Q 48 28 44 24 Q 40 30 34 26 Q 28 30 24 24 Q 20 28 16 22 Z" fill={color} stroke={STROKE} strokeWidth="2" />
...
export function CartoonPerson({
  ...
  const aL = pose ? pose.armL : armLeftAngle
  const aR = pose ? pose.armR : armRightAngle
  const lL = pose ? pose.legL : legLeftAngle
  const lR = pose ? pose.legR : legRightAngle
  const bY = pose ? pose.bodyY : 0
  const bRot = pose ? pose.bodyRotate : 0

  const viewBox: React.CSSProperties = { transformBox: 'view-box' as const }
  ...
  return (
    <div className="cartoon-person toca-style" title={label}>
      <svg width="68" height="100" viewBox="0 0 68 100" fill="none" xmlns="http://www.w3.org/2000/svg">
        <motion.g
          animate={{ y: bY, rotate: bRot }}
          transition={SPRING}
          style={{ ...viewBox, transformOrigin: '34px 50px' }}
        >
          ...
          <motion.g
            animate={{ rotate: aL }}
            transition={SPRING}
            style={{ ...viewBox, transformOrigin: '18px 48px' }}
          >
          ...
          <motion.g
            animate={{ rotate: aR }}
            transition={SPRING}
            style={{ ...viewBox, transformOrigin: '50px 48px' }}
          >
          ...
          <motion.g
            animate={{ rotate: lL }}
            transition={SPRING}
            style={{ ...viewBox, transformOrigin: '26px 70px' }}
          >
          ...
          <motion.g
            animate={{ rotate: lR }}
            transition={SPRING}
            style={{ ...viewBox, transformOrigin: '42px 70px' }}
          >
